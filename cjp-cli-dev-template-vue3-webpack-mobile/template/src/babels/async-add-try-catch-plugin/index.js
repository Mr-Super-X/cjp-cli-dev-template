// babel-template 用于将字符串形式的代码来构建AST树节点，如果要使用es module开发该插件，请安装@babel/template
const template = require('babel-template')

const { tryTemplate, catchConsole, mergeOptions, matchesFile } = require('./util')

/**
 * 给async/await添加try/catch处理的loader
 * @param {*} param0
 * @returns
 *
 * 使用方法：
 *
 *  配置 Babel
 *  babel.config.js中添加配置plugins: ['./src/babel/async-add-try-catch-loader']
 */
module.exports = function (babel) {
  ...
  const visitor = {
    AwaitExpression(path) {
      ...
      if (this.opts && !typeof this.opts === 'object') {
        return console.error('[async-add-try-catch-loader]: options need to be an object.')
      }
      ...
      const asyncPath = path.findParent(
        p =>
          p.node.async &&
          (p.isFunctionDeclaration() || p.isArrowFunctionExpression() || p.isFunctionExpression() || p.isObjectMethod())
      )
  ...
  return {
    name: 'async-add-try-catch-loader',
    visitor,
  }
}
